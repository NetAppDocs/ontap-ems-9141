...
[.lead]
dbs.assignments events are presented when a Distributed Block Store (DBS) assignments event is reported, which occurs when the distribution of block data within the cluster changes, and no corrective action is typically required.
