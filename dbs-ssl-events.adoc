...
[.lead]
dbs.ssl events are presented when Distributed Block Store (DBS) detects that a Secure Socket Layer (SSL) certificate on a node is nearing expiration, and no corrective action is typically required.
