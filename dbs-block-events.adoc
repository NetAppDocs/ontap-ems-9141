...
[.lead]
dbs.block events are presented when a Distributed Block Store (DBS) successfully completes a block service repair operation involving block rewrites to all BServices that are replicas for the block, and corrective action is to Contact NetApp technical support for data related events to assess the severity and for any required corrective actions.
