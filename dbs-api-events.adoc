...
[.lead]
dbs.api events are presented when a Distributed Block Store (DBS) event related to an API operation occurs, such as an authentication failure, and no corrective action is typically required.
