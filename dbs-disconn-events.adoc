...
[.lead]
dbs.disconn EMS events occur when ONTAP encounters issues associated with Distributed Block Store (DBS) cluster connectivity, such as a disconnected cluster pair or an unreachable remote node.
