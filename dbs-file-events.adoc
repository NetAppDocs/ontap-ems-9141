...
[.lead]
dbs.file EMS events occur when ONTAP encounters issues associated with the mounted container file system, such as low capacity or an unexpected read-only state.
