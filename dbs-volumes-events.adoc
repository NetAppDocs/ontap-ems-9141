...
[.lead]
dbs.volumes events are presented when the distributed block store detects that the listed volumes currently have only one copy of some of their data (they do not have a live secondary), and corrective action is to Check for network connectivity issues and hardware errors.
