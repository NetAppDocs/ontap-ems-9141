...
[.lead]
dbs.discon events are presented when the Distributed Block Store (DBS) is unable to access snapmirror endpoint on the network, and corrective action is to Check network connectivity between the cluster and the remote snapmirror endpoint.
