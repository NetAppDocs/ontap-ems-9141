...
[.lead]
master.couldnt EMS events occur when ONTAP encounters issues associated with a node's inability to cast a vote or demote the current cluster master in the global database.
