...
[.lead]
dbs.raid events are presented when distributed block store detects that a RAID group in the cluster is degraded, and corrective action is to Look for disk error EMS messages, or use REST, Kubernetes, or kubectl calls to check for disk errors.
