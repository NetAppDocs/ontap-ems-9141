...
[.lead]
This message occurs when the destination volume of a SnapMirror(R) relationship is not of -type Data Protection (DP) and a 'snapmirror initialize' operation is attempted.
