...
[.lead]
dbs.remote events are presented when a Distributed Block Store (DBS) event such as a change in cluster pair connectivity status occurs, and no corrective action is typically required.
