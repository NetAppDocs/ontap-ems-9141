...
[.lead]
ffrespmon.aggr events are presented when an aggregate is detected to be offline due to an unresponsive Firefly Service on a node, and no corrective action is typically required.
