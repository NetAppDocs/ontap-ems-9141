...
[.lead]
This message occurs when a SnapMirror(R) resync operation detects data divergence between the common Snapshot(tm) copy and the AFS on a resync destination volume where the destination volume is a SnapLock Compliance volume and the SnapMirror relationship is of "async-mirror" policy type.
