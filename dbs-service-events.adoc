...
[.lead]
dbs.service EMS events occur when ONTAP encounters issues associated with Distributed Block Store (DBS) service availability, such as a service failing to start or not running.
