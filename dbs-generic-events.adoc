...
[.lead]
dbs.generic events are presented when a Distributed Block Store (DBS) cluster fault is reported as a generic fault, and no corrective action is typically required.
